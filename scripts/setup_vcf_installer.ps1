...
if($VCFOperationsManagerProperties -ne $null) {
    $VCFOperationsManagerProperties.GetEnumerator() | Foreach-Object {
        $script += "echo $($_.key)=$($_.value) >> ${vcfOperationsConfigFile}`n"
    }
}
...
while(1) {
    try {
        $requests = Invoke-WebRequest -Uri "https://${VCFInstallerFQDN}/vcf-installer-ui/login" -Method GET -SkipCertificateCheck -TimeoutSec 30
...
$request = Invoke-WebRequest -Uri https://${VCFInstallerFQDN}/v1/system/settings/depot -Method PUT -Body $depotBody -Headers $headers -SkipCertificateCheck -TimeoutSec 60
